...
const nspFP63AlburyCSV = (await fs.readFile(path.join(__dirname, 'sample-data', 'albury.tsv'))).toString().replace(/^\uFEFF/, '').split('\n').map(line => line.split('\t'))
...
describe('The PDF Table Reader class', () => {
  ...
  it('Should convert a PDF buffer into an array of pages, each containing the table data (albury data)', async () => {
    ...
    expect(pages[0]).to.deep.equal(nspFP63AlburyCSV)
  })
})