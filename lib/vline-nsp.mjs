...
class NSPFile {
  ...
  async download(outputDir) {
    let request = await fetch(constants.VLINE_CORPORATE_HOST + this.href)
    let outputPipe = fs.createWriteStream(path.join(outputDir, `${this.#nspVersion.version} ${this.name}.pdf`))
    request.pipe(outputPipe)
  }
